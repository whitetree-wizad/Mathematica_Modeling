...
\end{enumerate}
另外，由于产品A、B、C的加工过程需占用关键设备，消耗的时间不可忽略，记为工时$c^r(\texe{组件}r\in{\text{A,B,C}})$。一般地，为维持符号的同一性，方便表示和向量化，记$c^r=0,\texe{组件}r\notin{\text{A,B,C}}$.
...
$s^{r} \omega_{t}^{r}$表示部件$r$生产的生产准备费用，其中，$\omega_{t}^{r}$是为了表述在$t$时段是否生产组件$r$，而引用的0$\textendash$1变量
\begin{equation}
...
\begin{itemize}
	\item 库存数量\\
	组件$r$在第$t$天的库存量，应当是前一日的库存量$y_{t-1}^{r}$与当日组装数量$x_{t}^{r}$之和，再减去当日为生产组件$r'$而消耗的数量
	\begin{equation}
	...
	\item 生产总工时\\
	产品A、B、C的加工过程需占用关键设备，因而严格控制总生产工时，满足下式
...
\end{itemize}
	...
	由于式\ref{key1}不便直接用程序计算，故将式\ref{key1}、\ref{key2}合并地表为
	\begin{equation}
